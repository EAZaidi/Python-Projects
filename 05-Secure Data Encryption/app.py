...
def encrypt_text(plain_text, passkey):
    encrypted_bytes = xor_encrypt_decrypt(plain_text.encode(), passkey)
    return base64.b64encode(encrypted_bytes).decode()

def decrypt_text(encrypted_text, passkey):
    try:
        encrypted_bytes = base64.b64decode(encrypted_text.encode())
        decrypted_bytes = xor_encrypt_decrypt(encrypted_bytes, passkey)
...
# ============== UI FUNCTIONS ==============
def show_home():
    st.title("🔐 Secure Vault (No Cryptography)")
    st.success(f"Logged in as: {st.session_state.current_user}")
...
def show_store_data():
    ...
    if st.button("Encrypt & Save"):
        if text_to_store and passkey:
            encrypted_text = encrypt_text(text_to_store, passkey)
            passkey_hash = hash_passkey(passkey)
            st.session_state.stored_data["users"][st.session_state.current_user]["data"][encrypted_text] = {
                "encrypted_text": encrypted_text,
                "passkey": passkey_hash
